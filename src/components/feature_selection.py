...
# Function for feature selection using Univariate Feature Selection (ANOVA F-value)
def feature_selection(data: pd.DataFrame, target: pd.Series, num_features: int = 6) -> pd.DataFrame:
    """
    Selects the most important features using Univariate Feature Selection with ANOVA F-value.
    
    Parameters:
        data (pd.DataFrame): Input feature data.
        target (pd.Series): Target variable.
        num_features (int): Number of top features to select (default is 7).
        
    Returns:
        pd.DataFrame: Data with selected top features.
    """
    logger.info("Selecting features using Univariate Feature Selection (ANOVA F-value)...")
...
if __name__ == "__main__":
    ...
    processed_data = feature_selection(data, target, num_features=6)
